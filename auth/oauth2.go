...
func (params OAuth2) oauthRequest(opt Options, data url.Values) (map[string]interface{}, error) {
	if params.ClientID == "" {
		return nil, &types.ErrorResponse{Severity: types.ErrSeverityFatal, DataCode: types.ErrDataCodeMissingClientID, Err: fmt.Errorf("client_id cannot be empty")}
	}
	...
	if opt.DataInQuery {
		...
		if err != nil {
			return nil, &types.ErrorResponse{Err: err, DataCode: types.ErrDataCodeError}
		}
		rawQuery := u.RawQuery
		if rawQuery != "" {
			rawQuery += "&"
	...
	if err != nil {
		return nil, &types.ErrorResponse{HttpCode: response.StatusCode, DataCode: types.ErrDataCodeError, Err: fmt.Errorf("error while connecting to %s: %w", params.TokenEndpoint, err)}
	}

	if response.StatusCode == http.StatusTooManyRequests {
		return nil, &types.ErrorResponse{HttpCode: response.StatusCode, DataCode: types.ErrDataCodeTooManyRequest, Err: throttler.ErrTooManyRequests}
	}

	if response.Headers["Content-Type"] == nil {
		return nil, &types.ErrorResponse{HttpCode: response.StatusCode, DataCode: types.ErrDataCodeMissingContentTypeHeader, Err: fmt.Errorf("content-type header missing in response: %s", response.Body)}
	}
	...
	if strings.Contains(contentType, "application/json") {
		...
		if !ok {
			return nil, &types.ErrorResponse{HttpCode: response.StatusCode, DataCode: types.ErrDataCodeDeserialiseFailed, Err: fmt.Errorf("failed to deserialise the response: %s", response.Body)}
		}
		return parsed, nil
	}
	...
	return nil, &types.ErrorResponse{HttpCode: response.StatusCode, DataCode: types.ErrDataCodeResponseFormatUnknown, Err: fmt.Errorf("could not determine content type encoding from response")}
}
...
func (params OAuth2) Authorize(opt Options) (map[string]interface{}, error) {
	...
	if params.Code == "" {
		return nil, &types.ErrorResponse{Severity: types.ErrSeverityFatal, DataCode: types.ErrDataCodeMissingAuthorizationCode, Err: fmt.Errorf("the authorization code is missing")}
	}
...
func (params OAuth2) Refresh(opt Options) (map[string]interface{}, error) {
	...
	if params.RefreshToken == "" {
		return nil, &types.ErrorResponse{Severity: types.ErrSeverityFatal, DataCode: types.ErrDataCodeMissingRefreshToken, Err: fmt.Errorf("the refresh token is missing")}
	}
